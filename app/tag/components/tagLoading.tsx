...
const SearchLoading = (props : Iprops) => {
    return (
        <>
        <div className="page-title flex justify-between gap-1 mb-7 pb-2 border-b border-gray-100">
            <h3 className="text-gray-200 dark:text-gray-700">نتایج یافت شده برای عبارت «<span className="text-gray-200 dark:text-gray-700">{decodeURI(props.tagText)}</span>»</h3>
            <span title="جستجو دوباره" className="cursor-pointer text-blue-600 hover:text-blue-400">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-6 h-6 ${props.isRefetching ? "animate-spin" : ""}`}>
